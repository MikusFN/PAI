...
public class LearnManager : MonoBehaviour
{
    ...
    void Start()
    {
        genetic = GetComponent<GeneticAlgorithm>();
    ...
    void FixedUpdate()
    {
        ...
        float newDistance;
        Vector3 direction;
        //Debug.DrawLine(carBrain.transform.position, track.GetClosestPoint(carBrain.transform.position, out direction, out newDistance, 30), Color.red);
    ...
    void Load()
    {
        ...
        for (int i = 19; i < 19 + genetic.m_populationSize; ++i)
        {
            ...
            population.Add(new Genome(weights, float.Parse(data[i][1])));
        }
    }
    ...
    void Save()
    {
        ...
        int count = 0;
        foreach (Genome i in genetic.m_population)
        {
            data = new List<string>();
